...
class SpaceStoiciometryTests: XCTestCase {
    ...
    func testTrillionFuel1() {
        ...
        XCTAssertEqual(result, 82892753)
    }

    func testTrillionFuel2() {
        let input =
        """
        2 VPVL, 7 FWMGM, 2 CXFTF, 11 MNCFX => 1 STKFG
        17 NVRVD, 3 JNWZP => 8 VPVL
        ...
        """
        let result = findFuel(ore: 1000000000000, input: input)
        XCTAssertEqual(result, 5586022)
    }

    func testTrillionFuel3() {
        let input =
        """
        171 ORE => 8 CNZTR
